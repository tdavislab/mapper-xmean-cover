...
'''
Contains Graph for both the enhanced mapper graph and classic 1D mapper graph.
'''
...
class AbstractGraph(ABC):
    ...
    def add_node(self, n):
        '''
        Add a node n to the graph. Must be of type Node or EnhancedNode
        '''
        pass
...
class Graph(AbstractGraph):
    ...
    def get_node(self, interval_index:int, cluster_index:int):
        '''
        Given an interval and cluster, returns the node
        '''
        for n in self.nodes:
    ...
    def get_interval_nodes(self, interval_index: int):
        '''
        Gets all nodes corresponding to the interval specified.
        '''
    ...
    def add_edge(self, node1: Node, node2: Node):
        assert node1 in self.nodes and node2 in self.nodes, 'Attempting to add an edge to nodes that do not exist'
        self.edges.add((node1, node2))

    def to_hard_clustering_set(self, X, intervals=None):
        '''
        Returns a hard clustering over points cloud X (n pts x d dimensions). If intervals is None, a hard clustering set over the whole graph is computed. If intervals are specified, only the nodes coming from intervals is checked. 
        @param: X - n x d np.ndarray
        @param: None or list(int)
        '''
        ...
        for i, m in enumerate(assignments):
            if len(m) > 1:
                ...
                final_assignments.append(int(np.argmin(np.linalg.norm(subset_centroids - X[i]))))
            elif len(m) ==  1:
                final_indicies.append(i)
                final_assignments.append(m[0])
            elif i in point_indices or intervals is None:
                final_indicies.append(i)
                final_assignments.append(int(np.argmin(np.linalg.norm(centroids_np - X[i]))))
...
class EnhancedGraph(AbstractGraph):
    '''
    Implementation of the enhanced mapper graph. 
    '''
    def __init__(self, verbose=False):
        super().__init__()
    ...
    def get_node(self, interval_index: int, cluster_index: int, sign: Sign):
        for n in self.nodes:
            if n.interval_index == interval_index and n.cluster_index == cluster_index and n.sign == sign:
                return n
    ...
    def set_function_val(self, node: EnhancedNode, val: float):
        assert node in self.nodes, 'Attempting to set a function value for a nonexist node'

...
def vis_graph(g, title:str, lens, save_loc, piechart_save=None, labels=None, cmap: str = 'jet', enhanced=False):
    ...
    nt = Network(notebook=True, height='100%', width='100%', heading=title)
...
def pyvis_visualize(g: AbstractGraph, title:str, fname:str, enhanced:bool = False, notebook:bool = True, cmap: str = 'autumn', physics: bool=True, labels=None):
    assert len(g.nodes) != 0
    ...
    if labels is not None:
        ...
        for i in range(len(labels_set)):
            label_colors[labels_set[i]] = color_map(i / (len(labels_set) - 1))
    ...
    for node in g.nodes:
        if labels is None:
            nt.add_node(node2compactrep(node, enhanced), label=' ', title=str(node), \
                color= rgb2hex(color_map((g.function[node] - min_fn_val)/fn_range)[:3]) if enhanced else 'blue')
        else:
            nt.add_node(node2compactrep(node, enhanced), label=' ', title=str(node), \
                color= rgb2hex(color_map((g.function[node] - min_fn_val)/fn_range)[:3]) if enhanced else 'blue')

...
def generate_mapper_graph(X: np.ndarray, lens: np.ndarray, cover: Cover, clusterer: object, enhanced: bool = False, cover_min: float = None, cover_max: float = None, refit_cover=False):
    '''
    Main mapper generation code.
    @param: X - n pts x d dimensions point cloud, np.ndarray
    @param: lens - n pts vector for the lens
    @param: cover - cover object
    @param: clusterer - sklearn clustering object
    @param: enhanced - Whether or not to compute the enhanced mapper graph. Defaults False.
    @param: cover_min - minimum value for the cover intervals. Defaults lens.min()
    @param: cover_max - maximum value for the cover intervals. Defaults lens.max()
    @param: refit_cover - Whether the cover object should be refit to X. If using the adaptive cover methods, Use False. Defaults True.
            If you're using adaptive mapper, you should already have a cover object. In which case, set refit_cover = False.
    '''
...
def generate_lens(X, proj, scale = "MinMax"):
    '''
    Helper method that can compute a lens. Only "MinMax" scaling (sklearn's version) is support for now
    @param: X - n pts x d dimensions np.ndarray
    @param: proj - one of sum, mean, median, max, min, std, l2norm
    '''
    if scale == 'MinMax':

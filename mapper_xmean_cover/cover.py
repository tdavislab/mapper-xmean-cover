...
class AbstractCover(ABC):
    ...
    def vis_last_split(self, iteration, loc):
        '''
        Debugging method that visualizes the last split performed.
        '''
        if self.last_split is None or iteration is None:
            return
        i = 0
        while i < self.intervals.shape[0]:
            green = False
            if i == self.last_split or i == self.last_split + 1:
                green = True
            plt.plot(self.intervals[i], [self.intervals.shape[0] - i, self.intervals.shape[0] - i], 'g' if green else 'k')
            i = i + 1
        plt.yticks([])
        plt.title(f'Iteration {iteration}')
        plt.savefig(f'{loc}/Intermediate-{iteration}.jpg')
        plt.close()
        plt.clf()
    ...
    def remove_duplicate_cover_elements(self, debug=False):
        ...
        for d in marked_for_deletion:
            if d in intervals_list:  # Might have duplicates due to numerical quirks - float comparison
...
class CentroidCover(AbstractCover):
    '''
    Given a set of centroids, creates a cover from the centroids.
    '''
    ...
    def compute_intervals(self, bound_min: int, bound_max: int, lens: np.ndarray = None):
        # Intervals are computed during initalization
        pass
